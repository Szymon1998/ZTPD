...
SELECT SDO_LRS.VALID_MEASURE(GEOM, 500) VALID_500 FROM A6_LRS A;

--02B

SELECT SDO_LRS.GEOM_SEGMENT_END_PT(GEOM) END_PT FROM A6_LRS A;

--02C

SELECT SDO_LRS.LOCATE_PT(A.GEOM, 150, 0) KM150 FROM A6_LRS A;

--02D

SELECT SDO_LRS.CLIP_GEOM_SEGMENT(A.GEOM, 120, 160) CLIPPED FROM A6_LRS A;
...
SELECT SDO_GEOM.SDO_LENGTH(SDO_LRS.OFFSET_GEOM_SEGMENT(A6.GEOM, M.DIMINFO, 50, 200, 50, 'unit=m'), 1, 'unit=m') KOSZT
